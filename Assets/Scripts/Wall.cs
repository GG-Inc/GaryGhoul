...
public class Wall : MonoBehaviour {
	...
	public void config(float doorPos) {
		...
		if (doorPos < 1) {
			components [1].transform.localScale = new Vector3 (1 - (doorPos + DOOR_WIDTH), 1, DOOR_HEIGHT);
			components [1].transform.localPosition = new Vector3 ((1 + doorPos + DOOR_WIDTH) / 2, 0, -DOOR_HEIGHT / 2) * 10;
		}
		components [2].transform.localScale = new Vector3 (1, 1, 1 - DOOR_HEIGHT);
		components [2].transform.localPosition = new Vector3 (.5f, 0, -(1 + DOOR_HEIGHT) / 2) * 10;
	}
